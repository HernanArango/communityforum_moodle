...
define('COMMUNITYFORUM_MODE_FLATOLDEST', 1);
define('COMMUNITYFORUM_MODE_FLATNEWEST', -1);
define('COMMUNITYFORUM_MODE_THREADED', 2);
define('COMMUNITYFORUM_MODE_NESTED', 3);

define('COMMUNITYFORUM_CHOOSESUBSCRIBE', 0);
define('COMMUNITYFORUM_FORCESUBSCRIBE', 1);
define('COMMUNITYFORUM_INITIALSUBSCRIBE', 2);
define('COMMUNITYFORUM_DISALLOWSUBSCRIBE',3);

/**
 * COMMUNITYFORUM_TRACKING_OFF - Tracking is not available for this forum.
 */
define('COMMUNITYFORUM_TRACKING_OFF', 0);

/**
 * COMMUNITYFORUM_TRACKING_OPTIONAL - Tracking is based on user preference.
 */
define('COMMUNITYFORUM_TRACKING_OPTIONAL', 1);

/**
 * communityforum_TRACKING_FORCED - Tracking is on, regardless of user setting.
 * Treated as communityforum_TRACKING_OPTIONAL if $CFG->communityforum_allowforcedreadtracking is off.
 */
define('COMMUNITYFORUM_TRACKING_FORCED', 2);

define('COMMUNITYFORUM_MAILED_PENDING', 0);
define('COMMUNITYFORUM_MAILED_SUCCESS', 1);
define('COMMUNITYFORUM_MAILED_ERROR', 2);

if (!defined('COMMUNITYFORUM_CRON_USER_CACHE')) {
    /** Defines how many full user records are cached in forum cron. */
    define('COMMUNITYFORUM_CRON_USER_CACHE', 5000);
}

/**
 * COMMUNITYFORUM_POSTS_ALL_USER_GROUPS - All the posts in groups where the user is enrolled.
 */
define('COMMUNITYFORUM_POSTS_ALL_USER_GROUPS', -2);

define('COMMUNITYFORUM_DISCUSSION_PINNED', 1);
define('COMMUNITYFORUM_DISCUSSION_UNPINNED', 0);
 ...
 * @param mod_communityforum_mod_form $mform
 * @return int intance id
 */
function communityforum_add_instance($forum, $mform = null) {
    global $CFG, $DB;
    ...
    $forum->id = $DB->insert_record('communityforum', $forum);
    ...
    if ($forum->type == 'single') {  // Create related discussion.
        ...
        $discussion->id = communityforum_add_discussion($discussion, null, $message);

        if ($mform and $draftid = file_get_submitted_draft_itemid('introeditor')) {
            // Ugly hack - we need to copy the files somehow.
            $discussion = $DB->get_record('communityforum_discussions', array('id'=>$discussion->id), '*', MUST_EXIST);
    ...
    communityforum_grade_item_update($forum);
 ...
 * @return bool success
 */
function communityforum_update_instance($forum, $mform) {
    ...
    if (($oldforum->assessed<>$forum->assessed) or ($oldforum->scale<>$forum->scale)) {
        communityforum_update_grades($forum); // recalculate grades for the forum
    }
...
function communityforum_delete_instance($id) {
    ...
    $DB->delete_records('communityforum_digests', array('forum' => $forum->id));
    $DB->delete_records('communityforum_subscriptions', array('forum'=>$forum->id));
    $DB->delete_records('communityforum_discuss_subs', array('forum' => $forum->id));

    if ($discussions = $DB->get_records('communityforum_discussions', array('forum'=>$forum->id))) {
        foreach ($discussions as $discussion) {
            if (!communityforum_delete_discussion($discussion, true, $course, $cm, $forum)) {
                $result = false;
            }
        }
    }

    communityforum_tp_delete_read_records(-1, -1, -1, $forum->id);

    if (!$DB->delete_records('forum', array('id'=>$forum->id))) {
        $result = false;
    }

    communityforum_grade_item_delete($forum);
 ...
 */
function communityforum_supports($feature) {
    switch($feature) {
 ...
 */
function communityforum_get_completion_state($course,$cm,$userid,$type) {
    global $CFG,$DB;
...
SELECT
    COUNT(1)
FROM
    {communityforum_posts} fp
    INNER JOIN {communityforum_discussions} fd ON fp.discussion=fd.id
WHERE
    fp.userid=:userid AND fd.forum=:forumid";

    if ($forum->completiondiscussions) {
        $value = $forum->completiondiscussions <=
                 $DB->count_records('communityforum_discussions',array('forum'=>$forum->id,'userid'=>$userid));
 ...
 */
function communityforum_get_email_message_id($postid, $usertoid) {
 ...
 */
function communityforum_cron_minimise_user_record(stdClass $user) {
...
function communityforum_cron() {
    ...
    $digestsset = $DB->get_recordset('communityforum_digests', null, '', 'id, userid, forum, maildigest');
    $digests = array();
    ...
    $messageinboundgenerator->set_handler('\mod_communityforum\message\inbound\reply_handler');

    if ($posts = communityforum_get_unmailed_posts($starttime, $endtime, $timenow)) {
        // Mark them all now as being mailed.  It's unlikely but possible there
        // might be an error later so that a post is NOT actually mailed out,
        // but since mail isn't crucial, we can accept this risk.  Doing it now
        // prevents the risk of duplicated mails, which is a worse problem.

        if (!communityforum_mark_old_posts_as_mailed($endtime)) {
                ...
                if ($subusers = \mod_communityforum\subscriptions::fetch_subscribed_users($forums[$forumid], 0, $modcontext, 'u.*', true)) {
                    ...
                    foreach ($subusers as $postuser) {
                        ...
                        if ($userscount > COMMUNITYFORUM_CRON_USER_CACHE) {
                            ...
                            communityforum_cron_minimise_user_record($postuser);
    ...
    if ($users && $posts) {
        ...
        foreach ($users as $userto) {
            ...
            if (isset($userto->username)) {
                ...
                communityforum_cron_minimise_user_record($userto);
            }
            ...
            foreach ($posts as $pid => $post) {
                ...
                if ($forum->type == 'qanda' && !communityforum_get_user_posted_time($discussion->id, $userto->id) && $pid != $discussion->firstpost) {
                ...
                if (!isset($userto->canpost[$discussion->id])) {
                    $modcontext = context_module::instance($cm->id);
                    $userto->canpost[$discussion->id] = communityforum_user_can_post($forum, $discussion, $userto, $cm, $course, $modcontext);
                }
                ...
                $maildigest = communityforum_get_user_maildigest_bulk($digests, $userto, $forum->id);
                ...
                if ($maildigest > 0) {
                    ...
                    $DB->insert_record('communityforum_queue', $queue);
                    continue;
                }
                ...
                $userfrom->customheaders = array (
                    ...
                    'List-Help: '       . $CFG->wwwroot . '/mod/communityforum/view.php?f=' . $forum->id,
                    'Message-ID: '      . communityforum_get_email_message_id($post->id, $userto->id),
                ...
                if (!isset($userto->canpost[$discussion->id])) {
                    $canreply = communityforum_user_can_post($forum, $discussion, $userto, $cm, $course, $modcontext);
                } else {
                ...
                $rootid = communityforum_get_email_message_id($discussion->firstpost, $userto->id);

                if ($post->parent) {
                    // This post is a reply, so add reply header (RFC 2822).
                    $parentid = communityforum_get_email_message_id($post->parent, $userto->id);
                ...
                if (!$mailresult) {
                    mtrace("Error: mod/communityforum/lib.php communityforum_cron(): Could not send out mail for id $post->id to user $userto->id".
                            " ($userto->email) .. not trying again.");
                    $errorcount[$post->id]++;
                } else {
                    $mailcount[$post->id]++;

                    // Mark post as read if communityforum_usermarksread is set off.
                    if (!$CFG->communityforum_usermarksread) {
            ...
            if (get_user_preferences('communityforum_markasreadonnotification', 1, $userto->id) == 1) {
                communityforum_tp_mark_posts_read($userto, $userto->markposts);
            }
    ...
    $DB->delete_records_select('communityforum_queue', "timemodified < ?", array($weekago));
    ...
    if ($CFG->digestmailtimelast < $digesttime and $timenow > $digesttime) {
        ...
        $digestposts_rs = $DB->get_recordset_select('communityforum_queue', "timemodified < ?", array($digesttime));
        ...
        if ($digestposts_rs->valid()) {
            ...
            foreach ($digestposts_rs as $digestpost) {
                if (!isset($posts[$digestpost->postid])) {
                    if ($post = $DB->get_record('communityforum_posts', array('id' => $digestpost->postid))) {
                ...
                if (!isset($discussions[$discussionid])) {
                    if ($discussion = $DB->get_record('communityforum_discussions', array('id' => $discussionid))) {
                        $discussions[$discussionid] = $discussion;
                    } else {
                        continue;
                    }
            ...
            foreach ($userdiscussions as $userid => $thesediscussions) {
                ...
                $DB->delete_records_select('communityforum_queue', "userid = ? AND timemodified < ?", array($userid, $digesttime));
                ...
                if (!$mailresult) {
                    mtrace("ERROR: mod/communityforum/cron.php: Could not send out digest mail to user $userto->id ".
                        "($userto->email)... not trying again.");
                } else {
                    mtrace("success.");
                    $usermailcount++;
    ...
    if (!empty($CFG->communityforum_lastreadclean)) {
        $timenow = time();
        if ($CFG->communityforum_lastreadclean + (24*3600) < $timenow) {
            set_config('communityforum_lastreadclean', $timenow);
            mtrace('Removing old forum read tracking info...');
            communityforum_tp_clean_read_records();
        }
    } else {
        set_config('communityforum_lastreadclean', time());
    }
 ...
 */
function communityforum_user_outline($course, $user, $mod, $forum) {
    global $CFG;
    require_once("$CFG->libdir/gradelib.php");
    $grades = grade_get_grades($course->id, 'mod', 'communityforum', $forum->id, $user->id);
    ...
    $count = communityforum_count_user_posts($forum->id, $user->id);
 ...
 * @param object $forum
 */
function communityforum_user_complete($course, $user, $mod, $forum) {
    ...
    if ($posts = communityforum_get_user_posts($forum->id, $user->id)) {

        if (!$cm = get_coursemodule_from_instance('forum', $forum->id, $course->id)) {
            print_error('invalidcoursemodule');
        }
        $discussions = communityforum_get_user_involved_discussions($forum->id, $user->id);
 ...
 */
function communityforum_filter_user_groups_discussions($discussions) {
 ...
 * @return bool
 */
function communityforum_is_user_group_discussion(cm_info $cm, $discussiongroupid) {
 ...
 */
function communityforum_print_overview($courses,&$htmlarray) {
    ...
    $forumsnewposts = communityforum_filter_user_groups_discussions($discussions);

    // also get all forum tracking stuff ONCE.
    $trackingforums = array();
    foreach ($forums as $forum) {
        if (communityforum_tp_can_track_forums($forum)) {
            $trackingforums[$forum->id] = $forum;
        }
    }

    if (count($trackingforums) > 0) {
        $cutoffdate = isset($CFG->communityforum_oldpostdays) ? (time() - ($CFG->communityforum_oldpostdays*24*60*60)) : 0;
        $sql = 'SELECT d.forum,d.course,COUNT(p.id) AS count '.
            ' FROM {communityforum_posts} p '.
            ' JOIN {communityforum_discussions} d ON p.discussion = d.id '.
            ' LEFT JOIN {communityforum_read} r ON r.postid = p.id AND r.userid = ? WHERE (';
 ...
 * @return bool success
 */
function communityforum_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    foreach ($posts as $post) {
        ...
        if (communityforum_is_user_group_discussion($cm, $post->groupid)) {
            $printposts[] = $post;
        }
    ...
    foreach ($printposts as $post) {
        ...
        $authorhidden = communityforum_is_author_hidden($post, (object) ['type' => $post->forumtype]);
 ...
 */
function communityforum_get_user_grades($forum, $userid = 0) {
    global $CFG;
    ...
    $ratingoptions->itemtable = 'communityforum_posts';
 ...
 * @return void
 */
function communityforum_update_grades($forum, $userid=0, $nullifnone=true) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
    ...
    } else if ($grades = communityforum_get_user_grades($forum, $userid)) {
        communityforum_grade_item_update($forum, $grades);

    } else if ($userid and $nullifnone) {
        $grade = new stdClass();
        $grade->userid   = $userid;
        $grade->rawgrade = NULL;
        communityforum_grade_item_update($forum, $grade);
 ...
 * @return int 0 if ok
 */
function communityforum_grade_item_update($forum, $grades=NULL) {
    global $CFG;
 ...
 * @return grade_item
 */
function communityforum_grade_item_delete($forum) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');
 ...
 * @return bool
 */
function communityforum_scale_used ($forumid,$scaleid) {
    global $DB;
    $return = false;
 ...
 */
function communityforum_scale_used_anywhere($scaleid) {
    global $DB;
...
/**
 * Gets a post with all info ready for communityforum_print_post
 ...
 */
function communityforum_get_post_full($postid) {
    global $CFG, $DB;
 ...
 */
function communityforum_get_all_discussion_posts($discussionid, $sort, $tracking=false) {
    global $CFG, $DB, $USER;
    ...
    if ($tracking) {
        ...
        $tr_join = "LEFT JOIN {communityforum_read} fr ON (fr.postid = p.id AND fr.userid = ?)";
        $params[] = $USER->id;
    }
    ...
    foreach ($posts as $pid=>$p) {
        if ($tracking) {
            if (communityforum_tp_is_post_old($p)) {
 ...
 */
function communityforum_get_readable_forums($userid, $courseid=0) {
    ...
    foreach ($courses as $course) {
        ...
        /// qanda access
            if ($forum->type == 'qanda'
                ...
                if ($discussionspostedin = communityforum_discussions_user_has_posted_in($forum->id, $USER->id)) {
 ...
 */
function communityforum_search_posts($searchterms, $courseid=0, $limitfrom=0, $limitnum=50,
                            &$totalcount, $extrasql='') {
    global $CFG, $DB, $USER;
    require_once($CFG->libdir.'/searchlib.php');

    $forums = communityforum_get_readable_forums($USER->id, $courseid);
    ...
    $fromsql = "{communityforum_posts} p,
                  {communityforum_discussions} d,
                  {user} u";
 ...
 * @return array
 */
function communityforum_get_unmailed_posts($starttime, $endtime, $now=null) {
    global $CFG, $DB;
 ...
 * @return bool
 */
function communityforum_mark_old_posts_as_mailed($endtime, $now=null) {
    global $CFG, $DB;
...
/**
 * Get all the posts for a user in a forum suitable for communityforum_print_post
 *
 * @global object
 * @global object
 * @uses CONTEXT_MODULE
 * @return array
 */
function communityforum_get_user_posts($forumid, $userid) {
    global $CFG, $DB;
 ...
 */
function communityforum_get_user_involved_discussions($forumid, $userid) {
    global $CFG, $DB;
 ...
 */
function communityforum_count_user_posts($forumid, $userid) {
    global $CFG, $DB;
 ...
 * @return array|null
 */
function communityforum_get_post_from_log($log) {
    global $CFG, $DB;
 ...
 * @return array
 */
function communityforum_get_firstpost_from_discussion($discussionid) {
    global $CFG, $DB;
 ...
 * @return array
 */
function communityforum_count_discussion_replies($forumid, $forumsort="", $limit=-1, $page=-1, $perpage=0) {
    global $CFG, $DB;
 ...
 * @return mixed
 */
function communityforum_count_discussions($forum, $cm, $course) {
    global $CFG, $DB, $USER;
 ...
 *                     Use communityforum_POSTS_ALL_USER_GROUPS for all the user groups
...
function communityforum_get_discussions($cm, $category,$forumsort="", $fullpost=true, $unused=-1, $limit=-1,
    ...
    if (empty($forumsort)) {
        $forumsort = communityforum_get_default_sort_order();
    }
    if (empty($fullpost)) {
 ...
 */
function communityforum_get_discussion_neighbours($cm, $discussion, $forum) {
    global $CFG, $DB, $USER;
    ...
    $sql = "SELECT d.id, d.name, d.timemodified, d.groupid, d.timestart, d.timeend
              FROM {communityforum_discussions} d
              JOIN {communityforum_posts} p ON d.firstpost = p.id
    ...
    $orderbydesc = communityforum_get_default_sort_order(true, $comparefield, 'd', false);
    $orderbyasc = communityforum_get_default_sort_order(false, $comparefield, 'd', false);

    if ($forum->type === 'blog') {
         $subselect = "SELECT pp.created
                   FROM {communityforum_discussions} dd
                   JOIN {communityforum_posts} pp ON dd.firstpost = pp.id ";
 ...
 * @return string
 */
function communityforum_get_default_sort_order($desc = true, $compare = 'd.timemodified', $prefix = 'd', $pinned = true) {
    global $CFG;
 ...
 * @return array
 */
function communityforum_get_discussions_unread($cm) {
    global $CFG, $DB, $USER;

    $now = round(time(), -2);
    $cutoffdate = $now - ($CFG->communityforum_oldpostdays*24*60*60);
    ...
    $sql = "SELECT d.id, COUNT(p.id) AS unread
              FROM {communityforum_discussions} d
                   JOIN {communityforum_posts} p     ON p.discussion = d.id
                   LEFT JOIN {communityforum_read} r ON (r.postid = p.id AND r.userid = $USER->id)
 ...
 * @param string $type
 */
function communityforum_get_course_forum($courseid, $type) {
 ...
 * @global object
 * @global object
 * @uses communityforum_MODE_THREADED
 ...
 * @param boolean $dummyifcantsee When communityforum_user_can_see_post says that
 ...
 * @return void
 */
function communityforum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if ($istracked && is_null($postisread)) {
        $postisread = communityforum_tp_is_post_read($USER->id, $post);
    }
    ...
    list($attachments, $attachedimages) = communityforum_print_attachments($post, $cm, 'separateimages');

    // Determine if we need to shorten this post
    $shortenpost = ($link && (strlen(strip_tags($post->message)) > $CFG->communityforum_longpost));
    ...
    if ($istracked && $CFG->communityforum_usermarksread && isloggedin()) {
    ...
    $authorhidden = communityforum_is_author_hidden($post, $forum);
    ...
    if ($link) {
        ...
        if ($post->replies == 1) {
            $replystring = get_string('repliesone', 'communityforum', $post->replies);
        } else {
            $replystring = get_string('repliesmany', 'communityforum', $post->replies);
        }
        if (!empty($discussion->unread) && $discussion->unread !== '-') {
            $replystring .= ' <span class="sep">/</span> <span class="unread">';
            if ($discussion->unread == 1) {
                $replystring .= get_string('unreadpostsone', 'communityforum');
            } else {
                $replystring .= get_string('unreadpostsnumber', 'communityforum', $discussion->unread);
            }
            $replystring .= '</span>';
        }

        $output .= html_writer::start_tag('div', array('class'=>'link'));
        $output .= html_writer::link($discussionlink, get_string($langstring, 'communityforum'));
        $output .= '&nbsp;('.$replystring.')';
    ...
    if ($istracked && !$CFG->communityforum_usermarksread && !$postisread) {
        communityforum_tp_mark_post_read($USER->id, $post, $forum->id);
    }
...
function communityforum_rating_validate($params) {
    ...
    // Fetch all the related records ... we need to do this anyway to call communityforum_user_can_see_post
    $post = $DB->get_record('communityforum_posts', array('id' => $params['itemid'], 'userid' => $params['rateduserid']), '*', MUST_EXIST);
    $discussion = $DB->get_record('communityforum_discussions', array('id' => $post->discussion), '*', MUST_EXIST);
    ...
    if (!communityforum_user_can_see_post($forum, $discussion, $post, $USER, $cm)) {
        throw new rating_exception('nopermissiontorate');
    }
...
function mod_communityforum_rating_can_see_item_ratings($params) {
    ...
    $post = $DB->get_record('communityforum_posts', array('id' => $params['itemid']), '*', MUST_EXIST);
    $discussion = $DB->get_record('communityforum_discussions', array('id' => $post->discussion), '*', MUST_EXIST);
    $forum = $DB->get_record('communityforum', array('id' => $discussion->forum), '*', MUST_EXIST);
    $course = $DB->get_record('course', array('id' => $forum->course), '*', MUST_EXIST);
    $cm = get_coursemodule_from_instance('communityforum', $forum->id, $course->id , false, MUST_EXIST);
 ...
 * that calls this one: communityforum_print_latest_discussions()
 *
 * @global object
 * @global object
 ...
 */
function communityforum_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
    ...
    if (!isset($rowcount)) {
        $rowcount = 0;
        $strmarkalldread = get_string('markalldread', 'communityforum');
    } else {
    ...
    $timeddiscussion = !empty($CFG->communityforum_enabletimedposts) && ($post->timestart || $post->timeend);
    $timedoutsidewindow = '';
    ...
    echo '<td class="'.$topicclass.' table-posts">';
    if (COMMUNITYFORUM_DISCUSSION_PINNED == $post->pinned) {
        echo $OUTPUT->pix_icon('i/pinned', get_string('discussionpinned', 'communityforum'), 'mod_communityforum');
    }
    ...
    echo '<a class="title-posts-table" href="'.$CFG->wwwroot.'/mod/communityforum/discuss.php?d='.$post->discussion.'">'.$post->subject.'</a><br>';
    $post_t = $DB->get_records('communityforum_posts', array('id'=>$post->id));
    $message_post="";
    foreach ($post_t as $post_r) {
        $message_post=$post_r->message;
        }
    if (strlen($message_post) > 300){
        $read_more="<strong><a href=".$CFG->wwwroot."/mod/communityforum/discuss.php?d=".$post->discussion.">".get_string('readmore','communityforum')."</a></strong>";
        $cut_message=substr($message_post,0,280)."...".$read_more;
        echo $cut_message;      
    }
    else{
        echo $message_post;
 ...
 */
function communityforum_get_discussion_subscription_icon($forum, $discussionid, $returnurl = null, $includetext = false) {
 ...
 * @return string The generated markup
 */
function communityforum_get_discussion_subscription_icon_preloaders() {
    $o = '';
...
function communityforum_print_mode_form($id, $mode, $forumtype='') {
    ...
    if ($forumtype == 'single') {
        ...
        $select->set_label(get_string('displaymode', 'communityforum'), array('class' => 'accesshide'));
 ...
 * @global object
 */
function communityforum_set_return() {
    global $CFG, $SESSION;
 ...
 * @return string
 */
function communityforum_go_back_to($default) {
    global $SESSION;
 ...
 * @return bool success
 */
function communityforum_move_attachments($discussion, $forumfrom, $forumto) {
    global $DB;

    $fs = get_file_storage();

    $newcm = get_coursemodule_from_instance('communityforum', $forumto);
    $oldcm = get_coursemodule_from_instance('communityforum', $forumfrom);
 ...
 */
function communityforum_print_attachments($post, $cm, $type) {
    ...
    $strattachment = get_string('attachment', 'communityforum');
 ...
 */
function communityforum_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    global $CFG, $DB, $USER;
    ...
    if (isset($cached['post']) && $cached['post']->id == $itemid) {
        $post = $cached['post'];
    } else if ($post = $DB->get_record('communityforum_posts', array('id' => $itemid))) {
        $cached['post'] = $post;
    } else {
        return null;
    }
    ...
    } else if ($discussion = $DB->get_record('communityforum_discussions', array('id' => $post->discussion))) {
    ...
    if (isset($cached['forum']) && $cached['forum']->id == $cm->instance) {
        $forum = $cached['forum'];
    } else if ($forum = $DB->get_record('communityforum', array('id' => $cm->instance))) {
        $cached['forum'] = $forum;
    } else {
        return null;
    }
 ...
 */
function communityforum_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $CFG, $DB;
    ...
    $areas = communityforum_get_file_areas($course, $cm, $context);
    ...
    if (!$post = $DB->get_record('communityforum_posts', array('id'=>$postid))) {
        return false;
    }
 ...
 * @return bool
 */
function communityforum_add_attachment($post, $forum, $cm, $mform=null, $unused=null) {
    global $DB;
 ...
 * @return int
 */
function communityforum_add_new_post($post, $mform, $unused = null) {
    global $USER, $DB;
    ...
    $DB->set_field("communityforum_discussions", "timemodified", $post->modified, array("id" => $post->discussion));
    $DB->set_field("communityforum_discussions", "usermodified", $post->userid, array("id" => $post->discussion));

    if (communityforum_tp_can_track_forums($forum) && communityforum_tp_is_tracked($forum)) {
        communityforum_tp_mark_post_read($post->userid, $post, $post->forum);
    }

    // Let Moodle know that assessable content is uploaded (eg for plagiarism detection)
    communityforum_trigger_content_uploaded_event($post, $cm, 'communityforum_add_new_post');
 ...
 * @return  bool
 */
function communityforum_update_post($newpost, $mform, $unused = null) {
    global $DB, $USER;
    ...
    // Let Moodle know that assessable content is uploaded (eg for plagiarism detection)
    communityforum_trigger_content_uploaded_event($post, $cm, 'communityforum_update_post');
 ...
 * @return object
 */
function communityforum_add_discussion($discussion, $mform=null, $unused=null, $userid=null) {
    global $USER, $CFG, $DB;
    ...
    $post->discussion = $DB->insert_record("communityforum_discussions", $discussion);

    // Finally, set the pointer on the post.
    $DB->set_field("communityforum_posts", "discussion", $post->discussion, array("id"=>$post->id));

    if (!empty($cm->id)) {
        communityforum_add_attachment($post, $forum, $cm, $mform, $unused);
    }
    ...
    // Let Moodle know that assessable content is uploaded (eg for plagiarism detection)
    if (!empty($cm->id)) {
        communityforum_trigger_content_uploaded_event($post, $cm, 'communityforum_add_discussion');
    }
 ...
 * @return bool
 */
function communityforum_delete_discussion($discussion, $fulldelete, $course, $cm, $forum) {
    global $DB, $CFG;
    require_once($CFG->libdir.'/completionlib.php');

    $result = true;

    if ($posts = $DB->get_records("communityforum_posts", array("discussion" => $discussion->id))) {
        foreach ($posts as $post) {
            $post->course = $discussion->course;
            $post->forum  = $discussion->forum;
            if (!communityforum_delete_post($post, 'ignore', $course, $cm, $forum, $fulldelete)) {
                $result = false;
            }
        }
    }
 ...
 * @return bool
 */
function communityforum_delete_post($post, $children, $course, $cm, $forum, $skipcompletion=false) {
    global $DB, $CFG, $USER;
    require_once($CFG->libdir.'/completionlib.php');

    $context = context_module::instance($cm->id);

    if ($children !== 'ignore' && ($childposts = $DB->get_records('communityforum_posts', array('parent'=>$post->id)))) {
       if ($children) {
           foreach ($childposts as $childpost) {
               communityforum_delete_post($childpost, true, $course, $cm, $forum, $skipcompletion);
           }
       } else {
           return false;
       }
    ...
    if ($DB->delete_records("communityforum_posts", array("id" => $post->id))) {

        communityforum_tp_delete_read_records(-1, $post->id);

    // Just in case we are deleting the last post
        communityforum_discussion_update_last_post($post->discussion);
 ...
 * @return int
 */
function communityforum_count_replies($post, $children=true) {
    global $DB;
    $count = 0;

    if ($children) {
        if ($childposts = $DB->get_records('communityforum_posts', array('parent' => $post->id))) {
           foreach ($childposts as $childpost) {
               $count ++;                   // For this child
               $count += communityforum_count_replies($childpost, true);
           }
        }
    } else {
        $count += $DB->count_records('communityforum_posts', array('parent' => $post->id));
    }
 ...
 * @return string
 */
function communityforum_get_subscribe_link($forum, $context, $messages = array(), $cantaccessagroup = false, $fakelink=true, $backtoindex=false, $subscribed_forums=null) {
 ...
 * @return bool
 */
function communityforum_user_has_posted_discussion($forumid, $userid, $groupid = null) {
    global $CFG, $DB;

    $sql = "SELECT 'x'
              FROM {communityforum_discussions} d, {communityforum_posts} p
 ...
 * @return array
 */
function communityforum_discussions_user_has_posted_in($forumid, $userid) {
    global $CFG, $DB;
 ...
 * @return bool
 */
function communityforum_user_has_posted($forumid, $did, $userid) {
    global $DB;
    ...
    if (empty($did)) {
        ...
        return $DB->record_exists('communityforum_posts', array('discussion'=>$did,'userid'=>$userid));
    }
}
 ...
 */
function communityforum_get_user_posted_time($did, $userid) {
    global $DB;

    $posttime = $DB->get_field('communityforum_posts', 'MIN(created)', array('userid'=>$userid, 'discussion'=>$did));
    if (empty($posttime)) {
        return false;
    }
    return $posttime;
 ...
 * @return bool
 */
function communityforum_user_can_post_discussion($forum, $currentgroup=null, $unused=-1, $cm=NULL, $context=NULL) {
// $forum is an object
    global $USER;
    ...
    if ($forum->type == 'eachuser') {
        if (communityforum_user_has_posted_discussion($forum->id, $USER->id, $currentgroup)) {
            return false;
        }
    }
 ...
 * discussion. Use communityforum_user_can_post_discussion() to check whether the user
 ...
 * @return bool
 */
function communityforum_user_can_post($forum, $discussion, $user=NULL, $cm=NULL, $course=NULL, $context=NULL) {
    global $USER, $DB;
    if (empty($user)) {
        $user = $USER;
    }
...
*/
function communityforum_user_can_see_timed_discussion($discussion, $user, $context) {
    global $CFG;
...
* @return boolean returns true if they can view post, false otherwise
*/
function communityforum_user_can_see_group_discussion($discussion, $cm, $context) {
 ...
 * @return bool
 */
function communityforum_user_can_see_discussion($forum, $discussion, $context, $user=NULL) {
    global $USER, $DB;
    ...
    if (is_numeric($discussion)) {
        ...
        if (!$discussion = $DB->get_record('communityforum_discussions',array('id'=>$discussion))) {
            return false;
        }
    }
    if (!$cm = get_coursemodule_from_instance('communityforum', $forum->id, $forum->course)) {
    ...
    if (!communityforum_user_can_see_timed_discussion($discussion, $user, $context)) {
        return false;
    }

    if (!communityforum_user_can_see_group_discussion($discussion, $cm, $context)) {
        return false;
    }
 ...
 * @return bool
 */
function communityforum_user_can_see_post($forum, $discussion, $post, $user=NULL, $cm=NULL) {
    global $CFG, $USER, $DB;
    ...
    if (is_numeric($post)) {
        ...
        if (!$post = $DB->get_record('communityforum_posts',array('id'=>$post))) {
            return false;
        }
    }
    ...
    if (!communityforum_user_can_see_timed_discussion($discussion, $user, $modcontext)) {
        return false;
    }

    if (!communityforum_user_can_see_group_discussion($discussion, $cm, $modcontext)) {
        return false;
    }

    if ($forum->type == 'qanda') {
        $firstpost = communityforum_get_firstpost_from_discussion($discussion->id);
        $userfirstpost = communityforum_get_user_posted_time($discussion->id, $user->id);
...
function communityforum_print_latest_discussions($course, $forum, $category,$maxdiscussions = -1, $displayformat = 'plain', $sort = '',
    ...
    if (empty($sort)) {
        $sort = communityforum_get_default_sort_order();
    }
...
// and the current user is a guest.
    ...
    $canstart = communityforum_user_can_post_discussion($forum, $currentgroup, $groupmode, $cm, $context);
...
// If we want paging
    if ($page != -1) {
        ...
        if ($numdiscussions > 1000) {
            ...
            $replies = communityforum_count_discussion_replies($forum->id, $sort, $maxdiscussions, $page, $perpage);
        } else {
            $replies = communityforum_count_discussion_replies($forum->id);
        }
    ...
    if ($cantrack = communityforum_tp_can_track_forums($forum)) {
    ...
    if ($forumtracked) {
        $unreads = communityforum_get_discussions_unread($cm);
    } else {
        $unreads = array();
    }
    ...
    foreach ($discussions as $discussion) {
        ...
        switch ($displayformat) {
            case 'header':
                ...
                communityforum_print_discussion_header($discussion, $forum, $group, $strdatestring, $cantrack, $forumtracked,
            ...
            default:
                ...
                if ($discussion->replies) {
                    ...
                    $modcontext = context_module::instance($cm->id);
                    $link = communityforum_user_can_see_discussion($forum, $discussion, $modcontext, $USER);
                }
 ...
 *
 * @uses CONTEXT_MODULE
 * @uses communityforum_MODE_FLATNEWEST
 * @uses communityforum_MODE_FLATOLDEST
 * @uses communityforum_MODE_THREADED
 * @uses communityforum_MODE_NESTED
 ...
 * @param bool $canrate
 */
function communityforum_print_discussion($course, $cm, $forum, $discussion, $post, $mode, $canreply=NULL, $canrate=false) {
    global $USER, $CFG;
    ...
    $modcontext = context_module::instance($cm->id);
    if ($canreply === NULL) {
        $reply = communityforum_user_can_post($forum, $discussion, $USER, $cm, $course, $modcontext);
    } else {
        $reply = $canreply;
    }
    ...
    $forumtracked = communityforum_tp_is_tracked($forum);
    $posts = communityforum_get_all_discussion_posts($discussion->id, $sort, $forumtracked);
    $post = $posts[$post->id];
    ...
    $post->forum = $forum->id;   // Add the forum id to the post object, later used by communityforum_print_post
    ...
    communityforum_print_post($post, $discussion, $forum, $cm, $course, $ownpost, $reply, false,
 ...
 * @return void
 */
function communityforum_print_posts_flat($course, &$cm, $forum, $discussion, $post, $mode, $reply, $forumtracked, $posts) {
    global $USER, $CFG;
    ...
    foreach ($posts as $post) {
        ...
        communityforum_print_post($post, $discussion, $forum, $cm, $course, $ownpost, $reply, $link,
                             '', '', $postread, true, $forumtracked);
    }
}
 ...
 * @return void
 */
function communityforum_print_posts_threaded($course, &$cm, $forum, $discussion, $parent, $depth, $reply, $forumtracked, $posts) {
    global $USER, $CFG;
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            communityforum_print_posts_threaded($course, $cm, $forum, $discussion, $post, $depth-1, $reply, $forumtracked, $posts);
            echo "</div>\n";
        }
    }
}
 ...
 * @return void
 */
function communityforum_print_posts_nested($course, &$cm, $forum, $discussion, $parent, $reply, $forumtracked, $posts) {
    global $USER, $CFG;
 ...
 * @global object
 */
function communityforum_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0)  {
 ...
 */
function communityforum_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    global $OUTPUT;
 ...
 * @return bool
 */
function communityforum_change_discussionid($postid, $discussionid) {
    global $DB;
    $DB->set_field('communityforum_posts', 'discussion', $discussionid, array('id' => $postid));
    if ($posts = $DB->get_records('communityforum_posts', array('parent' => $postid))) {
        foreach ($posts as $post) {
            communityforum_change_discussionid($post->id, $discussionid);
        }
    }
    return true;
}
 ...
 * @return string
 */
function communityforum_update_subscriptions_button($courseid, $forumid) {
    global $CFG, $USER;
 ...
 */
function communityforum_tp_mark_posts_read($user, $postids) {
    global $CFG, $DB;

    if (!communityforum_tp_can_track_forums(false, $user)) {
        return true;
    }
    ...
    $now = time();
    $cutoffdate = $now - ($CFG->communityforum_oldpostdays * 24 * 3600);
    ...
    if (empty($postids)) {
        ...
        while ($part = array_splice($postids, 0, 200)) {
            $status = communityforum_tp_mark_posts_read($user, $part) && $status;
        }
        return $status;
    }
    ...
    $sql = "INSERT INTO {communityforum_read} (userid, postid, discussionid, forumid, firstread, lastread)
    ...
    $status = $DB->set_field_select('communityforum_read', 'lastread', $now, '
                userid      =  :userid
 ...
 * @param int $postid
 */
function communityforum_tp_add_read_record($userid, $postid) {
    global $CFG, $DB;

    $now = time();
    $cutoffdate = $now - ($CFG->communityforum_oldpostdays * 24 * 3600);

    if (!$DB->record_exists('communityforum_read', array('userid' => $userid, 'postid' => $postid))) {
        $sql = "INSERT INTO {communityforum_read} (userid, postid, discussionid, forumid, firstread, lastread)
        ...
        $sql = "UPDATE {communityforum_read}
                   SET lastread = ?
 ...
 * @return bool
 */
function communityforum_tp_mark_post_read($userid, $post, $forumid) {
    if (!communityforum_tp_is_post_old($post)) {
        return communityforum_tp_add_read_record($userid, $post->id);
    } else {
        return true;
    }
}
 ...
 * @return bool
 */
function communityforum_tp_mark_communityforum_read($user, $forumid, $groupid=false) {
    global $CFG, $DB;

    $cutoffdate = time() - ($CFG->communityforum_oldpostdays*24*60*60);
    ...
    $sql = "SELECT p.id
              FROM {communityforum_posts} p
                   LEFT JOIN {communityforum_discussions} d ON d.id = p.discussion
    ...
    if ($posts = $DB->get_records_sql($sql, $params)) {
        ...
        return communityforum_tp_mark_posts_read($user, $postids);
    }
 ...
 * @return bool
 */
function communityforum_tp_mark_discussion_read($user, $discussionid) {
    global $CFG, $DB;
 ...
 * @param object $post
 */
function communityforum_tp_is_post_read($userid, $post) {
    global $DB;
    return (communityforum_tp_is_post_old($post) ||
            $DB->record_exists('communityforum_read', array('userid' => $userid, 'postid' => $post->id)));
}
 ...
 */
function communityforum_tp_is_post_old($post, $time=null) {
    global $CFG;

    if (is_null($time)) {
        $time = time();
    }
    return ($post->modified < ($time - ($CFG->communityforum_oldpostdays * 24 * 3600)));
}
 ...
 * @return array
 */
function communityforum_tp_get_course_unread_posts($userid, $courseid) {
    global $CFG, $DB;
 ...
 * @return int
 */
function communityforum_tp_count_communityforum_unread_posts($cm, $course) {
    global $CFG, $USER, $DB;
    ...
    if (!isset($readcache[$course->id])) {
        ...
        if ($counts = communityforum_tp_get_course_unread_posts($USER->id, $course->id)) {
            foreach ($counts as $count) {
 ...
 * @return bool
 */
function communityforum_tp_delete_read_records($userid=-1, $postid=-1, $discussionid=-1, $forumid=-1) {
    global $DB;
    $params = array();
    ...
    if ($select == '') {
        ...
        return $DB->delete_records_select('communityforum_read', $select, $params);
    }
}
/**
 ...
 */
function communityforum_tp_get_untracked_forums($userid, $courseid) {
    global $CFG, $DB;
 ...
 * @return boolean
 */
function communityforum_tp_can_track_forums($forum=false, $user=false) {
    global $USER, $CFG, $DB;

    // if possible, avoid expensive
    // queries
    if (empty($CFG->communityforum_trackreadposts)) {
        return false;
    }
 ...
 * @return boolean
 */
function communityforum_tp_is_tracked($forum, $user=false) {
    global $USER, $CFG, $DB;
    ...
    if (!communityforum_tp_can_track_forums($forum, $user)) {
        return false;
    }

    $forumallows = ($forum->trackingtype == communityforum_TRACKING_OPTIONAL);
    $forumforced = ($forum->trackingtype == communityforum_TRACKING_FORCED);
    $userpref = $DB->get_record('communityforum_track_prefs', array('userid' => $user->id, 'forumid' => $forum->id));
 ...
 * @param int $userid
 */
function communityforum_tp_start_tracking($forumid, $userid=false) {
    global $USER, $DB;
    ...
    return $DB->delete_records('communityforum_track_prefs', array('userid' => $userid, 'forumid' => $forumid));
}
 ...
 * @param int $userid
 */
function communityforum_tp_stop_tracking($forumid, $userid=false) {
    global $USER, $DB;
    ...
    if (!$DB->record_exists('communityforum_track_prefs', array('userid' => $userid, 'forumid' => $forumid))) {
        $track_prefs = new stdClass();
        $track_prefs->userid = $userid;
        $track_prefs->forumid = $forumid;
        $DB->insert_record('communityforum_track_prefs', $track_prefs);
    }

    return communityforum_tp_delete_read_records($userid, -1, -1, $forumid);
}


/**
 * Clean old records from the communityforum_read table.
 * @global object
 * @global object
 * @return void
 */
function communityforum_tp_clean_read_records() {
    global $CFG, $DB;

    if (!isset($CFG->communityforum_oldpostdays)) {
        return;
    }
// Look for records older than the cutoffdate that are still in the communityforum_read table.
    $cutoffdate = time() - ($CFG->communityforum_oldpostdays*24*60*60);

    //first get the oldest tracking present - we need tis to speedup the next delete query
    $sql = "SELECT MIN(fp.modified) AS first
              FROM {communityforum_posts} fp
                   JOIN {communityforum_read} fr ON fr.postid=fp.id";
 ...
 * @return bool|int
 **/
function communityforum_discussion_update_last_post($discussionid) {
    global $CFG, $DB;

// Check the given discussion exists
    if (!$DB->record_exists('communityforum_discussions', array('id' => $discussionid))) {
        return false;
    }

// Use SQL to find the last post for this discussion
    $sql = "SELECT id, userid, modified
              FROM {communityforum_posts}
             WHERE discussion=?
...
// Lets go find the last post
    if (($lastposts = $DB->get_records_sql($sql, array($discussionid), 0, 1))) {
        ...
        $DB->update_record('communityforum_discussions', $discussionobject);
        return $lastpost->id;
    }
 ...
 * @return array
 */
function communityforum_get_view_actions() {
 ...
 * @return array
 */
function communityforum_get_post_actions() {
 ...
 */
function communityforum_check_throttling($forum, $cm = null) {
    global $CFG, $DB, $USER;

    if (is_numeric($forum)) {
        $forum = $DB->get_record('communityforum', array('id' => $forum), '*', MUST_EXIST);
    }
    ...
    if (!$cm) {
        $cm = get_coursemodule_from_instance('communityforum', $forum->id, $forum->course, false, MUST_EXIST);
    }
    ...
    $numposts = $DB->count_records_sql('SELECT COUNT(p.id) FROM {communityforum_posts} p
                                        JOIN {communityforum_discussions} d
 ...
 *        from the function communityforum_check_throttling.
 */
function communityforum_check_blocking_threshold($thresholdwarning) {
 ...
 */
function communityforum_reset_gradebook($courseid, $type='') {
    global $CFG, $DB;
    ...
    if ($forums = $DB->get_records_sql($sql, $params)) {
        foreach ($forums as $forum) {
            communityforum_grade_item_update($forum, 'reset');
        }
    }
}
 ...
 */
function communityforum_reset_userdata($data) {
    global $CFG, $DB;
    require_once($CFG->dirroot.'/rating/lib.php');

    $componentstr = get_string('modulenameplural', 'communityforum');
    $status = array();
    ...
    $removeposts = false;
    $typesql     = "";
    if (!empty($data->reset_communityforum_all)) {
        $removeposts = true;
        $typesstr    = get_string('resetforumsall', 'communityforum');
        $types       = array();
    } else if (!empty($data->reset_communityforum_types)){
        $removeposts = true;
        $types       = array();
        $sqltypes    = array();
        $communityforum_types_all = communityforum_get_forum_types_all();
        foreach ($data->reset_communityforum_types as $type) {
            if (!array_key_exists($type, $communityforum_types_all)) {
                continue;
            }
            $types[] = $communityforum_types_all[$type];
            $sqltypes[] = $type;
        }
        if (!empty($sqltypes)) {
    ...
    if( $removeposts || !empty($data->reset_communityforum_ratings) ) {
    ...
    if ($removeposts) {
        ...
        $DB->delete_records_select('communityforum_read', "forumid IN ($forumssql)", $params);

        // remove tracking prefs
        $DB->delete_records_select('communityforum_track_prefs', "forumid IN ($forumssql)", $params);

        // remove posts from queue
        $DB->delete_records_select('communityforum_queue', "discussionid IN ($discussionssql)", $params);

        // all posts - initial posts must be kept in single simple discussion forums
        $DB->delete_records_select('communityforum_posts', "discussion IN ($discussionssql) AND parent <> 0", $params); // first all children
        $DB->delete_records_select('communityforum_posts', "discussion IN ($discussionssql AND f.type <> 'single') AND parent = 0", $params); // now the initial posts for non single simple

        // finally all discussions except single simple forums
        $DB->delete_records_select('communityforum_discussions', "forum IN ($forumssql AND f.type <> 'single')", $params);

        // remove all grades from gradebook
        if (empty($data->reset_gradebook_grades)) {
            if (empty($types)) {
                communityforum_reset_gradebook($data->courseid);
            } else {
                foreach ($types as $type) {
                    communityforum_reset_gradebook($data->courseid, $type);
                }
            }
        }
    ...
    if (!empty($data->reset_communityforum_ratings)) {
        if ($forums) {
            foreach ($forums as $forumid=>$unused) {
                if (!$cm = get_coursemodule_from_instance('forum', $forumid)) {
                    continue;
    ...
    if (!empty($data->reset_communityforum_digests)) {
        $DB->delete_records_select('communityforum_digests', "forum IN ($allforumssql)", $params);
    ...
    if (!empty($data->reset_communityforum_track_prefs)) {
        $DB->delete_records_select('communityforum_track_prefs', "forumid IN ($allforumssql)", $params);
 ...
 */
function communityforum_reset_course_form_definition(&$mform) {
    $mform->addElement('header', 'forumheader', get_string('modulenameplural', 'forum'));

    $mform->addElement('checkbox', 'reset_communityforum_all', get_string('resetforumsall','forum'));

    $mform->addElement('select', 'reset_communityforum_types', get_string('resetforums', 'forum'), communityforum_get_forum_types_all(), array('multiple' => 'multiple'));
    $mform->setAdvanced('reset_communityforum_types');
    $mform->disabledIf('reset_communityforum_types', 'reset_communityforum_all', 'checked');

    $mform->addElement('checkbox', 'reset_communityforum_digests', get_string('resetdigests','forum'));
    $mform->setAdvanced('reset_communityforum_digests');

    $mform->addElement('checkbox', 'reset_communityforum_subscriptions', get_string('resetsubscriptions','forum'));
    $mform->setAdvanced('reset_communityforum_subscriptions');

    $mform->addElement('checkbox', 'reset_communityforum_track_prefs', get_string('resettrackprefs','forum'));
    $mform->setAdvanced('reset_communityforum_track_prefs');
    $mform->disabledIf('reset_communityforum_track_prefs', 'reset_communityforum_all', 'checked');

    $mform->addElement('checkbox', 'reset_communityforum_ratings', get_string('deleteallratings'));
    $mform->disabledIf('reset_communityforum_ratings', 'reset_communityforum_all', 'checked');
}
 ...
 * @return array
 */
function communityforum_reset_course_form_defaults($course) {
    return array('reset_communityforum_all'=>1, 'reset_communityforum_digests' => 0, 'reset_communityforum_subscriptions'=>0, 'reset_communityforum_track_prefs'=>0, 'reset_communityforum_ratings'=>1);
}
 ...
 * @return array
 */
function communityforum_get_layout_modes() {
    return array (COMMUNITYFORUM_MODE_FLATOLDEST => get_string('modeflatoldestfirst', 'communityforum'),
                  COMMUNITYFORUM_MODE_FLATNEWEST => get_string('modeflatnewestfirst', 'communityforum'),
                  COMMUNITYFORUM_MODE_THREADED   => get_string('modethreaded', 'communityforum'),
                  COMMUNITYFORUM_MODE_NESTED     => get_string('modenested', 'communityforum'));
}
...
function communityforum_get_forum_types() {
    return array ('general'  => get_string('generalforum', 'communityforum'),
                  'eachuser' => get_string('eachuserforum', 'communityforum'),
                  'single'   => get_string('singleforum', 'communityforum'),
                  'qanda'    => get_string('qandaforum', 'communityforum'),
                  'blog'     => get_string('blogforum', 'communityforum'));
}
...
function communityforum_get_forum_types_all() {
    return array ('news'     => get_string('namenews','communityforum'),
                  'social'   => get_string('namesocial','communityforum'),
                  'general'  => get_string('generalforum', 'communityforum'),
                  'eachuser' => get_string('eachuserforum', 'communityforum'),
                  'single'   => get_string('singleforum', 'communityforum'),
                  'qanda'    => get_string('qandaforum', 'communityforum'),
                  'blog'     => get_string('blogforum', 'communityforum'));
 ...
 * @return array
 */
function communityforum_get_extra_capabilities() {
...
function communityforum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode) {
    ...
    $enablerssfeeds = !empty($CFG->enablerssfeeds) && !empty($CFG->communityforum_enablerssfeeds);
 ...
 */
function communityforum_cm_info_view(cm_info $cm) {
    global $CFG;

    if (communityforum_tp_can_track_forums()) {
        if ($unread = communityforum_tp_count_communityforum_unread_posts($cm, $cm->get_course())) {
 ...
 */
function communityforum_page_type_list($pagetype, $parentcontext, $currentcontext) {
    $communityforum_pagetype = array(
    ...
    return $communityforum_pagetype;
}
 ...
 */
function communityforum_get_courses_user_posted_in($user, $discussionsonly = false, $includecontexts = true, $limitfrom = null, $limitnum = null) {
    global $DB;

    // If we are only after discussions we need only look at the communityforum_discussions
    // table and join to the userid there. If we are looking for posts then we need
    // to join to the communityforum_posts table.
    if (!$discussionsonly) {
        $subquery = "(SELECT DISTINCT fd.course
                         FROM {communityforum_discussions} fd
                         JOIN {communityforum_posts} fp ON fp.discussion = fd.id
 ...
 */
function communityforum_get_forums_user_posted_in($user, array $courseids = null, $discussionsonly = false, $limitfrom = null, $limitnum = null) {
    global $DB;
    ...
    if ($discussionsonly) {
        $join = 'JOIN {communityforum_discussions} ff ON ff.forum = f.id';
    } else {
        $join = 'JOIN {communityforum_discussions} fd ON fd.forum = f.id
                 JOIN {communityforum_posts} ff ON ff.discussion = fd.id';
    }
 ...
 */
function communityforum_get_posts_by_user($user, array $courses, $musthaveaccess = false, $discussionsonly = false, $limitfrom = 0, $limitnum = 50) {
    global $DB, $USER, $CFG;
    ...
    $forums = communityforum_get_forums_user_posted_in($user, array_keys($return->courses), $discussionsonly);
    ...
    foreach ($return->courses as $course) {
        ...
        foreach ($modinfo->get_instances_of('forum') as $forumid => $cm) {
            ...
            // code were expecting an writeable object. See {@link communityforum_print_post()}.
    ...
    // We alias user.id to useridx because we communityforum_posts already has a userid field and not aliasing this would break
    // oracle and mssql.
    ...
    $sql = "FROM {communityforum_posts} p
            JOIN {communityforum_discussions} d ON d.id = p.discussion
            JOIN {user} u ON u.id = p.userid
           WHERE ($wheresql)
             AND p.userid = :userid ";
 ...
 */
function communityforum_set_user_maildigest($forum, $maildigest, $user = null) {
    global $DB, $USER;
    ...
    $subscription = $DB->get_record('communityforum_digests', array(
        'userid' => $user->id,
        'forum' => $forum->id,
    ));
    ...
    if ($subscription) {
        if ($maildigest == -1) {
            $DB->delete_records('communityforum_digests', array('forum' => $forum->id, 'userid' => $user->id));
            ...
            $DB->update_record('communityforum_digests', $subscription);
        }
    } else {
        if ($maildigest != -1) {
            ...
            $subscription->maildigest = $maildigest;
            $subscription->id = $DB->insert_record('communityforum_digests', $subscription);
        }
    }
}
 ...
 */
function communityforum_get_user_maildigest_bulk($digests, $user, $forumid) {
 ...
 */
function communityforum_get_user_digest_options($user = null) {
    global $USER;
 ...
 */
function communityforum_get_context($forumid, $context = null) {
    global $PAGE;
 ...
 * @since Moodle 2.9
 */
function communityforum_view($forum, $course, $cm, $context) {
 ...
 * @since Moodle 2.9
 */
function communityforum_discussion_view($modcontext, $forum, $discussion) {
    $params = array(
        'context' => $modcontext,
 ...
 * @return bool
 */
function mod_communityforum_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    if (isguestuser($user)) {
 ...
 * @throws coding_exception
 */
function communityforum_is_author_hidden($post, $forum) {
 ...
 * @return  string
 */
function mod_communityforum_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $PAGE;
 ...
 * @return  bool
 */
function communityforum_discussion_is_locked($forum, $discussion) {
 ...
 * @since Moodle 3.2
 */
function communityforum_check_updates_since(cm_info $cm, $from, $filter = array()) {
    ...
    $discussions = communityforum_get_discussions($cm, '', false, -1, -1, true, -1, 0, communityforum_POSTS_ALL_USER_GROUPS, $from);
    if (!empty($discussions)) {
