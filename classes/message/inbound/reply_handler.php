...
namespace mod_communityforum\message\inbound;
...
class reply_handler extends \core\message\inbound\handler {
    ...
    public function process_message(\stdClass $record, \stdClass $messagedata) {
        ...
        $post = $DB->get_record('communityforum_posts', array('id' => $record->datavalue));
        if (!$post) {
        ...
        $event->add_record_snapshot('communityforum_posts', $addpost);
