...
function xmldb_communityforum_upgrade($oldversion) {
    ...
    if ($oldversion < 2014051201) {
        ...
        foreach ($replacements as $old => $new) {
            $DB->set_field('communityforum', 'maxattachments', $new, array('maxattachments' => $old));
        }

        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2014051201, 'communityforum');
    }

    if ($oldversion < 2014081500) {

        // Define index course (not unique) to be added to forum_discussions.
        $table = new xmldb_table('communityforum_discussions');
        ...
        // communityforum savepoint reached.
        upgrade_mod_savepoint(true, 2014081500, 'communityforum');
    }

    if ($oldversion < 2014081900) {

        // Define table communityforum_discussion_subs to be created.
        $table = new xmldb_table('communityforum_discussion_subs');
        ...
        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2014081900, 'communityforum');
    }
    ...
    if ($oldversion < 2014103000) {
        ...
        if ($duplicatedrows = $DB->get_recordset_sql($sql)) {
            foreach ($duplicatedrows as $row) {
                $DB->delete_records_select('communityforum_read', 'userid = ? AND postid = ? AND id <> ?', array(
                    $row->userid,
                    $row->postid,
                    $row->lowid,
                ));
        ...
        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2014103000, 'communityforum');
    }
    ...
    if ($oldversion < 2014110300) {
        ...
        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2014110300, 'communityforum');
    }
    ...
    if ($oldversion < 2015102900) {
        ...
        $DB->set_field('communityforum_discussions', 'groupid', -1, array('groupid' => 0));

        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2015102900, 'communityforum');
    }
    ...
    if ($oldversion < 2015120800) {
        ...
        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2015120800, 'communityforum');
    }
    ...
    if ($oldversion < 2016091200) {
        ...
        $table = new xmldb_table('communityforum');
        ...
        // Forum savepoint reached.
        upgrade_mod_savepoint(true, 2016091200, 'communityforum');
    }
